...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Voice session registry: websocket connection_id -> VoiceSession
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Voice WebSocket endpoint (PCM 16kHz mono, optional auth below)
            self._app.router.add_get("/voice", self._handle_voice_ws)
    ...
    async def _handle_voice_ws(self, request: Any) -> Any:
        """Upgrade to WebSocket and run a voice session."""
        from gateway.voice_ws_handler import handle_voice_ws
        return await handle_voice_ws(self, request)

    # -----------------------------------------------------------------
    # Voice session helpers
    # -----------------------------------------------------------------
    @property
    def _voice_session_manager(self) -> Any:
        # Lazily attach a VoiceSessionManager so tests that construct the
        # adapter without a running gateway still get a working registry.
        if not hasattr(self, "_voice_mgr"):
            from gateway.voice_session import VoiceSessionManager
            self._voice_mgr = VoiceSessionManager()
